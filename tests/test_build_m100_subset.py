...
from src.build_m100_subset import build_subset, resolve_raw_root
...
def test_build_subset_ms_unit_index(monkeypatch, tmp_path) -> None:
    # Parquet timestamps from pyarrow are often in ms; verify that DataFrame alignment succeeds
    index = pd.date_range("2021-03-01", periods=3, freq="min", unit="ms", tz="UTC")
    ...
    def fake_read_metric(_root, _plugin, metric, _node):
        ...
        s = pd.Series(values[metric], index=index)
...
def test_resolve_raw_root_nested(tmp_path) -> None:
    outer = tmp_path / "download"
    inner = outer / "year_month=21-03"
    (inner / "plugin=ipmi_pub").mkdir(parents=True)
    assert resolve_raw_root(outer) == inner
    assert resolve_raw_root(inner) == inner
